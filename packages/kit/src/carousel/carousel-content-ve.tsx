...
export type CarouselContentProps = CarouselContentVEProps & React.ComponentProps<'div'>;

export const CarouselContentVE = React.forwardRef<
  HTMLDivElement,
  CarouselContentProps
>(({ children, onFocus, onBlur, onKeyDown, className, style, ...props }, ref) => {
  const {
    setTotalPages,
    itemsPerPage,
    totalPages,
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key === "ArrowLeft" && page > 0) {
      setPage(page - 1);
    } else if (event.key === "ArrowRight" && totalPages && page < totalPages - 1) {
      setPage(page + 1);
  ...
  React.useEffect(() => {
    if (!isTransitioning) {
      const firstVisible = findFirstVisibleItem(childRefs.current, internalRef.current);
      if (firstVisible) {
