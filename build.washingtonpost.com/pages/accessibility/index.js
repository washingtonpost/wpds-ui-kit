...
const Page = ({ wrapper }) => (
  <>
    <NextSeo title={`WPDS - Accessibility`} />
    <Header as="h1">Accessibility</Header>
    <Description>
      Learn how we consider accessibility in our workflow and how you can do so
      on your team. Watch our tutorials and workshops to discover the elegance
      and accessibility of WPDS.
    </Description>
    ...
    {wrapper.content.categories.map(
      ({ name, id, type, posts, size, description }) => (
        <React.Fragment key={name}>
          ...
          <SeeAllLink
            href={`/accessibility/${name.toLowerCase()}`}
            name={name}
            type={type}
          />
        </React.Fragment>
...
export const getStaticProps = async () => {
  const docs = await getDocsListBySection("accessibility");
