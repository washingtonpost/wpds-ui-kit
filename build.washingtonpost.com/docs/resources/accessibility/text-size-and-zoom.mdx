...
Avoid using complicated fonts that present words in unfamiliar or elaborate ways, such as in cursive or cartoon-ish styling. These can be difficult to read. Additionally, writing in all caps or with irregular capitalization can make text harder to read, especailly for screen reader users who may hear text read incorrectly do to irregular capitalization.

Our [WPDS Foundations fonts](/foundations/typography#Font%20size) are good examples of legible fonts. When using a non-WPDS font, consider the above to ensure it is accessible to users.

There is not a consensus as to whether serif or san-serif fonts are more readable. We at The Post use both.
