...
Within [our color tokens library](/foundations/color), there is a theme token labeled `accessible`, which maps to hex value `#666666` in light mode and `#858585` in dark mode. The contrast ratio is 5.7 when the `accessible` color is used in light mode on a pure white (`#FFFFFF`) background and in dark mode on a pure black (`#000000`) background.
...
View [our color tokens library](/foundations/color) — including the `accessible` token mentioned above — for more info.
