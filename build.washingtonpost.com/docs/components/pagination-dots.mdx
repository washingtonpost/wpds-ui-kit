...
If the dots are oriented left -> right or top -> bottom. It defaults to a horizontal orientation.
...
export default function Example() {
  return (
    ...
    >
      ...
      >
        <PaginationDots amount={6} index={1} label="pagination dots" orientation="vertical" />
      </Box>
    </Box>
  );
}
