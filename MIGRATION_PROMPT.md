...
export function validateThemeContrast(theme: typeof lightTheme) {
  ...
  if (primaryRatio < NORMAL_TEXT_RATIO) {
    issues.push(
      `Primary button contrast ${primaryRatio.toFixed(
        2
      )} < ${NORMAL_TEXT_RATIO}`
    );
  }
  ...
  colorPairs.forEach(([bg, fg]) => {
    ...
    if (ratio < NORMAL_TEXT_RATIO) {
      issues.push(
        `${bg}/${fg} contrast ${ratio.toFixed(2)} < ${NORMAL_TEXT_RATIO}`
      );
    }
  });
...
type ExtractSprinkleProps<T extends Record<string, any>> = {
  [K in keyof T]?: T[K] extends Record<string, any>
    ?
        | keyof T[K]
        | Partial<Record<keyof T[K]["conditions"], keyof T[K]["properties"][K]>>
    : keyof T[K];
};
    ...
    return (
      ...
      >
        {icon !== "none" && (
          <Icon
            label={
              icon === "center" ? children?.toString() || "Icon button" : ""
            }
            size="100"
...
async function generateCSSManifest() {
  ...
  for (const file of cssFiles) {
    if (file.endsWith(".css") && !file.includes(".css.ts")) {
      ...
      await fs.rename(path.join("dist", file), path.join("dist", hashedName));
...
async function extractCriticalCSS() {
  ...
  for (const file of cssFiles) {
    if (
      file.endsWith(".css") &&
      critical.core.some((comp) => file.includes(comp))
    ) {
      const content = await fs.readFile(path.join("dist", file), "utf8");
  ...
  await fs.writeFile("dist/critical.css", criticalCSS.join("\n"));
}

// Run build steps
async function build() {
  console.log(
    `Building styles in ${isProduction ? "production" : "development"} mode...`
  );
...
function validateBuild() {
  ...
  Object.entries(maxSizes).forEach(([file, maxSize]) => {
    ...
    if (stats.size > maxSize) {
      console.warn(
        `⚠️  ${file} exceeds size limit: ${stats.size} > ${maxSize}`
      );
    }
  });
    ...
    test.describe(`Story: ${storyId}`, () => {
      ...
      viewports.forEach((viewport) => {
        test(`Visual @ ${viewport.name}`, async ({ page }) => {
          ...
          // Test light mode
          await page.goto(
            `http://localhost:6006/iframe.html?id=${storyId}&theme=light`
          );
          await page.waitForLoadState("networkidle");
          ...
          // Test dark mode
          await page.goto(
            `http://localhost:6006/iframe.html?id=${storyId}&theme=dark`
          );
          await page.waitForLoadState("networkidle");
